...
class GaussSeidel:
    ...
    def _gauss_seidel_relaxation_iteration(self):
        for i in range(1, self.length//self.steplength):
            for j in range(1, self.length // self.steplength):
                self.grid[i, j] = 0.25*(self.grid[i+self.steplength, j] + self.grid[i-self.steplength, j]
                                            + self.grid[i, j-self.steplength] + self.grid[i, j+self.steplength])
        x = (self.grid[1:-1, 1:-1] - self.expected_grid) / self.expected_grid
        error = abs(max(x.min(), x.max(), key=abs))
        return error

    def calculate_potential(self, framedata):
        error = self._gauss_seidel_relaxation_iteration()
